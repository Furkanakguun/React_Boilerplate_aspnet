...
export interface ICreateOrUpdateStudentProps {
  visible: boolean;
  onCancel: () => void;
  modalType: string;
  onCreate: () => void;
...
class CreateOrUpdateStudent extends React.Component<ICreateOrUpdateStudentProps> {
  state = {
    confirmDirty: false,
  };
  ...
  render() {
    ...
    return (
      <Modal visible={visible} cancelText={L('Cancel')} okText={L('OK')} onCancel={onCancel} onOk={onCreate} title={'Student'} destroyOnClose={true}>
        <Form ref={this.props.formRef}>
          <Tabs defaultActiveKey={'studentInfo'} size={'small'} tabBarGutter={64}>
            <TabPane tab={'Student'} key={'studentInfo'}>
              ...
              <Form.Item label={L('Department')} {...formItemLayout} name={'department'} rules={rules.department}>
                <Input />
              </Form.Item>
