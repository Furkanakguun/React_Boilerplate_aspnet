...
import rules from './createOrUpdateDepartments.validation';
import { FormInstance } from 'antd/lib/form';

const TabPane = Tabs.TabPane;

export interface ICreateOrUpdateDepartmentProps {
  visible: boolean;
  onCancel: () => void;
  modalType: string;
  onCreate: () => void;
...
class CreateOrUpdateDepartment extends React.Component<ICreateOrUpdateDepartmentProps> {
  state = {
    confirmDirty: false,
  };
  ...
  render() {
    ...
    return (
      <Modal visible={visible} cancelText={L('Cancel')} okText={L('OK')} onCancel={onCancel} onOk={onCreate} title={'Department'} destroyOnClose={true}>
        <Form ref={this.props.formRef}>
          <Tabs defaultActiveKey={'departmentInfo'} size={'small'} tabBarGutter={64}>
            <TabPane tab={'Department'} key={'departmentInfo'}>
              <Form.Item label={L('Department name')} {...formItemLayout} name={'name'} rules={rules.name}>
                <Input />
              </Form.Item>
